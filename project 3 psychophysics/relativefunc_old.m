function [ sorted_new_bdiff,proportion,leftside_b,rightside_b] = relativefunc_old( color,time_threshold )
global new_bdiff
...
for i = 1:max_trial_no %Filling temp1
    ...
    if color == 4 % white case
        ...
        temp(sizex/2,(sizey/2)*1.2,:) = bias+(i-1)*((max_brightness/max_trial_no)+1);
        rightside_b = bias+(i-1)*((max_brightness/max_trial_no)+1);
        temp(sizex/2,(sizey/2)*0.8,:) = bias+(i-1)*((max_brightness/max_trial_no)+1) + new_bdiff(i);   
        ...
        temp(sizex/2,(sizey/2)*1.2,color) = bias+(i-1)*((max_brightness/max_trial_no)+1); % Brightness range 20 to 110.
        rightside_b = bias+(i-1)*((max_brightness/max_trial_no)+1);
        %left side: should be different
        temp(sizex/2,(sizey/2)*0.8,color) = bias+(i-1)*((max_brightness/max_trial_no)+1) + new_bdiff(i);
        leftside_b = bias+(i-1)*((max_brightness/max_trial_no)+1) + new_bdiff(i);
    end
...
score_diff = score_right - score_left ; % these are score differences
